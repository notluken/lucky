...
#mkdir ~/screenshots:while [[ condition ]]; do
...
# copy scripts
#cp -rv $RPATH/SCRIPTS/* ~/.config/polybar/bin/
...
chmod +x ~/.config/polybar/bin/target_to_hack.sh
chmod +x ~/.config/polybar/bin/hackthebox.sh
chmod +x ~/.config/polybar/bin/ethernet_status.sh
